...
public class MessageRefDao {
    ...
    public long searchLastCount(int userId){
        ...
        UpdateResult result = mongoTemplate.updateMulti(query, update, "message_ref");//全部更新
    ...
    public long updateUnreadMessage(String id){
        Query query = new Query();
        query.addCriteria(Criteria.where("_id").is(id));//根据主键查找只能找到一条数据
        Update update = new Update();
        update.set("readFlag", true);
        UpdateResult result = mongoTemplate.updateFirst(query, update, "message_ref");//只更第一条
        long rows = result.getModifiedCount();
        return rows;
    }
